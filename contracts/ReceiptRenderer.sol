...
/// @custom:oz-upgrades-from ReceiptRendererV0
contract ReceiptRenderer {
    using LibString for uint256;

    //solhint-disable quotes
    string private constant GREEN_SVG =
    ...
    function generateTokenURI(
        ...
        bytes memory dataURI =
            generateDataURI(tokenId_, questId_, totalParticipants_, claimed_, rewardAmount_, rewardAddress_);
    ...
    function generateSVG(
        bool claimed_,
        string memory rewardAmountString_,
        string memory rewardTokenSymbol_
    ) internal pure returns (string memory) {
        bytes memory svg = abi.encodePacked(
            claimed_ ? PURPLE_SVG : GREEN_SVG, rewardAmountString_, " ", rewardTokenSymbol_, "</text></svg>"
        );
    ...
    function humanRewardAmount(uint256 rewardAmount_, address rewardAddress_) external view returns (string memory) {
        uint8 decimals;
