...
describe("QuestFactory", () => {
  ...
  const sablierV2LockupLinearAddress =
  ...
  beforeEach(async () => {
    ...
    questTerminalKeyContract = await ethers.getContractFactory(
      "QuestTerminalKey"
    );
  ...
  const deployQuestTerminalKey = async () => {
    deployedQuestTerminalKeyContract = (await upgrades.deployProxy(
      questTerminalKeyContract,
      [
    ...
    deployedFactoryContract.setQuestTerminalKeyContract(
      deployedQuestTerminalKeyContract.address
    );
  };
  ...
  const deployFactoryContract = async () => {
    ...
    deployedFactoryContract = (await upgrades.deployProxy(
  ...
  describe("createQuestAndQueue()", () => {
    ...
    it("Should revert if reward address is not on the reward allowlist", async () => {
      ...
      expect(
        await deployedFactoryContract.rewardAllowlist(rewardAddress)
      ).to.equal(false);
    ...
    it("should create a new quest and start it", async () => {
      await deployedFactoryContract.setRewardAllowlistAddress(
        deployedSampleErc20Contract.address,
        true
      );
      ...
      const questAddress = await deployedFactoryContract
        .quests(erc20QuestId)
        .then((res) => res.questAddress);
      const deployedErc20Quest = await ethers.getContractAt(
        "Quest",
        questAddress
      );
      ...
      expect(
        await deployedSampleErc20Contract.balanceOf(questAddress)
    ...
    it("Should allow anyone to create a quest", async () => {
      ...
      await deployedSampleErc20Contract.transfer(
        royaltyRecipient.address,
        transferAmount
      );
      await deployedSampleErc20Contract
        .connect(royaltyRecipient)
        .approve(deployedFactoryContract.address, transferAmount);
      ...
      expect(await deployedErc20Quest.startTime()).to.equal(startDate);
      expect(await deployedErc20Quest.owner()).to.equal(
        royaltyRecipient.address
      );
    });
    ...
    it("createQuestAndQueue should create a new quest and start it with a discount", async () => {
      ...
      await deployedQuestTerminalKeyContract
        .connect(protocolRecipient)
        .mint(owner.address, 5000);
      const ids = await deployedQuestTerminalKeyContract.getOwnedTokenIds(
        owner.address
      );
      ...
      const maxProtocolRewardDiscounted =
        (maxTotalRewards * discountedQuestFee) / 10_000;
      const maxProtocolReward = (maxTotalRewards * questFee) / 10_000;
      const transferAmountDiscounted =
        maxTotalRewards + maxProtocolRewardDiscounted;
      const transferAmount = maxTotalRewards + maxProtocolReward;
      ...
      expect(
        await deployedSampleErc20Contract.balanceOf(questAddress)
      ).to.equal(transferAmountDiscounted);
      expect(await deployedErc20Quest.questFee()).to.equal(discountedQuestFee);
      expect(
        await deployedQuestTerminalKeyContract.discounts(discountTokenId)
      ).to.eql([5000, 1]); // percentage, usedCount
    });
  });
  ...
  describe("createERC20StreamQuest()", () => {
    ...
    it("createERC20StreamQuest should create a new quest and start it", async () => {
      ...
      const questAddress = await deployedFactoryContract
        .quests(erc20QuestId)
        .then((res) => res.questAddress);
      const erc20StreamQuest = await ethers.getContractAt(
        "Quest",
        questAddress
      );
  ...
  describe("setClaimSignerAddress()", () => {
    it("Should update claimSignerAddress", async () => {
      ...
      expect(await deployedFactoryContract.claimSignerAddress()).to.equal(
        newAddress
      );
    });
  });
  ...
  describe("setProtocolFeeRecipient()", () => {
    it("Should update protocolFeeRecipient", async () => {
      ...
      expect(await deployedFactoryContract.protocolFeeRecipient()).to.equal(
        newAddress
      );
    });
  ...
  describe("setQuestFee()", () => {
    ...
    it("Should revert if setting questFee beyond 10k basis points", async () => {
      ...
      await expect(
        deployedFactoryContract.setQuestFee(newQuestFee)
      ).to.be.revertedWithCustomError(questFactoryContract, "QuestFeeTooHigh");
    });
  });

  describe("getMintFeeRecipient", () => {
    it("Should return the protocolRecipient when no mintFeeRecipient is set", async () => {
      expect(await deployedFactoryContract.getMintFeeRecipient()).to.equal(
        protocolRecipient.address
      );
    });

    it("Should return the mintFeeRecipient when set", async () => {
      await deployedFactoryContract.setMintFeeRecipient(
  ...
  describe("questData()", () => {
    ...
    it("Should return the correct quest data for erc1155 quest", async () => {
      ...
      deployedSampleErc1155Contract.batchMint(
        owner.address,
        [NFTTokenId],
        [maxParticipants]
      );
      deployedSampleErc1155Contract.setApprovalForAll(
        deployedFactoryContract.address,
        true
      );
      ...
      await deployedFactoryContract
        .connect(questUser)
        .claim1155Rewards(erc1155QuestId, messageHash, signature);

      const questAddress = await deployedFactoryContract
        .quests(erc1155QuestId)
        .then((res) => res.questAddress);
  ...
  describe("claimRewards()", () => {
    ...
    beforeEach(async () => {
      messageHash = utils.solidityKeccak256(
        ["address", "string"],
        [questUser.address.toLowerCase(), erc20QuestId]
      );
      ...
      questAddress = await deployedFactoryContract
        .quests(erc20QuestId)
        .then((res) => res.questAddress);
    ...
    it("Should fail when trying to claim before quest time has started", async () => {
      ...
      await expect(
        deployedFactoryContract
          .connect(questUser)
          .claimRewards(erc20QuestId, messageHash, signature)
    ...
    it("Should claim rewards", async () => {
      ...
      await deployedFactoryContract
        ...
        .claimRewards(erc20QuestId, messageHash, signature);
      expect(
        await deployedSampleErc20Contract.balanceOf(questUser.address)
    ...
    it("should succeed if the mint fee is equal or greator than required amount, and only recieve the required amount", async function () {
      ...
      ).to.equal(balanceBefore.add(requiredFee));
    });
    ...
    it("should transfer referral fee percentage of mintFee to referral on erc20 quest", async function () {
      ...
      const affiliateBalanceBefore = await ethers.provider.getBalance(
        affiliate.address
      );
      ...
      await expect(
        deployedFactoryContract
          .connect(questUser)
          .claim(erc20QuestId, messageHash, signature, affiliate.address, {
            value: requiredFee + extraChange,
          })
      )
        .to.emit(deployedFactoryContract, "QuestClaimed")
      ...
      expect(await ethers.provider.getBalance(affiliate.address)).to.equal(
        affiliateBalanceBefore.add(referralAmount)
      );
    });
  });
  ...
  describe("claim1155Rewards()", () => {
    it("Should claim rewards from a 1155 Quest", async () => {
      ...
      expect(
        await deployedSampleErc1155Contract.balanceOf(
          questUser.address,
          NFTTokenId
        )
      ).to.equal(1);
    ...
    it("should claim rewards from a 1155 Quest with a referral", async () => {
      it("Should claim rewards from a 1155 Quest", async () => {
        ...
        await expect(
          deployedFactoryContract
            .connect(questUser)
            .claim(erc1155QuestId, messageHash, signature, affiliate.address, {
              value: requiredFee,
            })
        )
